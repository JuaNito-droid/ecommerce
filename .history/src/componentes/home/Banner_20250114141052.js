...
const Banner = ({ data = [] }) => {
  ...
  const transitionTime = 5000; // Tiempo total entre transiciones en milisegundos
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      }, 1500); // Duración del desvanecimiento en milisegundos
    }, transitionTime);

    return () => clearInterval(interval); // Limpia el temporizador al desmontar
  }, [data]);

  // Si `data` no es válido o está vacío, renderizar un mensaje o elemento predeterminado
  if (!Array.isArray(data) || data.length === 0) {
    return <div>No hay productos disponibles para mostrar.</div>;
  }

  // Obtener los productos visibles de manera segura
