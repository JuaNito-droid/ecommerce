...
const WishlistHeart = ({ addToWishlist, product, wishlist }) => {
    // Estado local para saber si el producto está en la lista de deseos
    const [isInWishlist, setIsInWishlist] = useState(false);

    useEffect(() => {
        // Comprobar si el producto está en la lista de deseos al cargar el componente
        if (wishlist && product) {
    ...
    }, [wishlist, product]); // Recalcular si cambia el wishlist o el producto

    const handleWishlistToggle = () => {
        // Cambiar el estado local
        setIsInWishlist((prev) => !prev);

        // Ejecutar la función que maneja la lógica de agregar/eliminar en la lista de deseos
