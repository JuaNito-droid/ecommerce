    ...
    groupedCategories.length > 1 ? (
      <li key={firstWord} className="relative">
        <div className="flex items-center h-5 my-5">
          <input
            ...
            onChange={() =>
              formData.category_id === firstWord
                ? onChange({ target: { name: "category_id", value: null } })  // Deseleccionar si ya está seleccionada
                : onChange({ target: { name: "category_id", value: firstWord } })
            }
          />
                ...
                <input
                  ...
                  onChange={() =>
                    formData.category_id === String(category.id)
                      ? onChange({ target: { name: "category_id", value: null } })  // Deseleccionar si ya está seleccionada
                      : onChange({ target: { name: "category_id", value: category.id } })
                  }
                />
      ...
      <li key={groupedCategories[0].id} className="flex items-center h-5 my-5">
        <input
          ...
          onChange={() =>
            formData.category_id === String(groupedCategories[0].id)
              ? onChange({ target: { name: "category_id", value: null } })  // Deseleccionar si ya está seleccionada
              : onChange({ target: { name: "category_id", value: groupedCategories[0].id } })
          }
        />
