      ...
      <Dialog as="div" className="fixed inset-0 flex z-40 lg:hidden" onClose={setMobileFiltersOpen}>
        ...
        >
          <div className="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto">
            ...
            {/* MOBILE FILTERS */}
            <form onSubmit={onSubmit} className="mt-4 border-t border-gray-200">
              <h3 className="sr-only">Categorias</h3>
              <ul className="font-medium text-gray-900 px-2 py-3">
                {categories &&
                  categories.map((category) => {
                    const renderSubCategories = () => {
                      return category.sub_categories.map((sub_category) => (
                    ...
                    return (
                      <div key={category.id}>
                        ...
                        {category.sub_categories.length > 0 && renderSubCategories()}
                      </div>
                    );
                  })}
              </ul>
