    ...
    groupedCategories.length > 1 ? (
      <li key={firstWord} className="relative">
        <div className="flex items-center h-5 my-5">
          <input
            ...
            onChange={(e) => {
              // Si la categoría seleccionada es la misma, deseleccionamos
              if (formData.category_id === firstWord) {
            ...
            {groupedCategories.map(category => (
              <li key={category.id} className="px-4 py-2 hover:bg-gray-100">
                <input
                  ...
                  onChange={(e) => {
                    // Si la categoría seleccionada es la misma, deseleccionamos
                    if (formData.category_id === String(category.id)) {
                      onChange({ target: { name: "category_id", value: "" } });
                    } else {
                      onChange(e);
                    }
      ...
      <li key={groupedCategories[0].id} className="flex items-center h-5 my-5">
        <input
          ...
          onChange={(e) => {
            // Si la categoría seleccionada es la misma, deseleccionamos
            if (formData.category_id === String(groupedCategories[0].id)) {
              onChange({ target: { name: "category_id", value: "" } });
            } else {
              onChange(e);
            }
