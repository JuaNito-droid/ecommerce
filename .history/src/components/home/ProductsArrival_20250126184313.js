          ...
          <Slider {...settings}>
            {data?.map((product) => (
              <div key={product.id} className="group relative px-2">
                <div
                  className="w-full h-52 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-52 lg:aspect-none border border-gray-300 p-1"
                >
                  <img
                    src={product.get_thumbnail}
                    alt="Product"
                    className="w-full h-full object-center object-cover rounded-sm p-2 m-5"
                  />
                </div>
                <div className="mt-5 flex justify-between">
                  ...
                  <p
                    className={`text-sm ${isAuthenticated ? "font-medium text-gray-900" : "text-red-500"}`}
                  >
