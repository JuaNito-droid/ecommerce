...
export const get_products_by_sold = () => async dispatch => {
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/product/get-products?sortBy=sold&order=desc&limit=4`, config);
...
export const get_products_for_showcase = () => async dispatch => {
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/product/get-products?sortBy=date_created&order=desc&limit=5`, config);
