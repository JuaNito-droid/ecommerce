...
def enviar_comentario(request):
    comentario = request.data.get('comentario', 'vr')
    if not comentario:
        return JsonResponse({'error': 'El comentario no puede estar vacío.'}, status=status.HTTP_400_BAD_REQUEST)
    
    try:
        send_mail(
            'Nuevo comentario desde el sitio web',
            comentario,
            'avega9982@gmail.com.com',  # Correo del remitente
            ['alefrvg@gmail.com.com'],  # Correo del destinatario
            fail_silently=False,
        )
        return JsonResponse({'message': 'Comentario enviado exitosamente.'}, status=status.HTTP_200_OK)
